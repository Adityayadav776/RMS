...
#============Register, Forget Password and Login Button=====================================

        btn_reg=Button(login_frame,text="Register New Account?",font=("times new roman",15),bg="white",bd=0,fg="#B00857",cursor="hand2",command=self.register_window).place(x=240,y=320)
        btn_forget=Button(login_frame,text="Forget Password?",font=("times new roman",15),bg="white",bd=0,fg="red",cursor="hand2",command=self.forget_password_window).place(x=450,y=320)
        btn_login=Button(login_frame,text="Login",font=("times new roman",20,"bold"),fg="white",bg="#15B01A",activebackground="yellowgreen",cursor="hand2",command=self.login).place(x=250,y=380,width=180,height=40)
...
#=====Forget password window================
    def forget_password(self):
        if self.cmb_quest.get()=="Select" or self.txt_answer.get()=="" or self.txt_new_pass.get()=="":
            ...
            try:
                con=pymysql.connect(hosts="localhost",user="root",password="",database="employee2")
                cur=con.cursor()
        ...
        if self.txt_email.get()=="":
            ...
            try:
                ...
                if row==None:
                    ...
                    self.root2.grab_set()
                    t=Label(self.root2,text="Forget Password",font=("times new roman",20,"bold"),bg="white",fb="red").place(x=0,y=10,rewidth=1)
#=======Forget password's security Questions and New Password=====================
                    ...
                    answer=Label(self.root2,text="New Password",font=("times new roman",15,"bold"),bg="white",fg="black").place(x=50,y=180)
                    self.txt_answer=Entry(self.root2,font=("times new roman",15),bg="lightgray")
                    self.txt_answer.place(x=50,y=210,width=250,)

                    new_password=Label(self.root2,text="Answer",font=("times new roman",15,"bold"),bg="white",fg="black").place(x=50,y=260)
