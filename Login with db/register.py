...
#==========Background Image=============================
        self.bg=ImageTk.PhotoImage(file=r"images/abc.jpg")
        bg=Label(self.root,image=self.bg).place(x=250,y=0,relwidth=1,relheight=1)


#==========Left Image=============================
        self.left=ImageTk.PhotoImage(file=r"images/10.jpg")
...
#========terms and condition=============
        ...
        btn_login=Button(self.root,text="Sign In",font=("times new roman",20),bg="lightblue",activebackground="green",cursor="hand2",command=self.login_window).place(x=195,y=325,width=160)
...
#=======Register Data Function=====================================================
    def register_data(self):
        if self.txt_fname.get()=="" or self.txt_contact.get()=="" or self.txt_email.get()=="" or self.cmb_quest.get()=="Select" or self.txt_answer.get()=="" or self.txt_password.get()=="" or self.txt_cpassword.get()=="":
                ...
                try:
                        con=pymsql.connect(host="localhost",user="root",password="",database="employee2")
                        cur=con.cursor()
                        ...
                        if row!=None:
                                ...
                                cur.execute("insert into emplyee (f_name,l_name,contact,email,question,answer,password) values(%s,%s,%s,%s,%s,%s,%s)",
                                        (self.txt_fname.get(),
                                        self.txt_lname.get(),
                        ...
                        messagebox.showerror("Enter",f"Error due to: {str(es)}",parent=self.root)
